...
import { Deposit } from '../../shared/interfaces/Deposit.interface';
import { DepositService } from '../../shared/Service/deposit/deposit.service';
import { FormDepositComponent } from './component/form-deposit/form-deposit.component';
...
export class DepositComponent implements OnInit {
  ...
  getinfoTable(){
    this.service.getList(this.filters).subscribe({
      next: (response: Deposit[]) => {
        ...
        this.infoTable.forEach((info) => {
          info.Date = new Date((info.Date as Date));
        });
      }, 
      error: (error) => {
        this.loading = false;
  ...
  new(){
    this.ref = this.dialogService.open(FormDepositComponent, {
      data: { action:'crear' },
      width: '90%',
      height: '100%',
